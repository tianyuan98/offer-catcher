...
3. original字段必须100%精确引用简历原文，一字不改、一词不换。即使原文有语法错误、口语化或不够专业，也必须原样复制，不得改写、缩写、扩写或替换同义词
4. 不得编造简历中不存在的模块（如没有"个人摘要"就不要硬造）
5. 只输出简历中真实存在的模块的优化对比
6. optimized版本必须使用STAR法则，保留原文真实量化数据，使用强动词

【第一人称绝对禁止】：
- optimized中禁止使用"我"、"我主导"、"我负责"、"我设计"等第一人称
- 动词直接用第三人称结构：「主导X」「统筹X」「设计X」「推动X」

【避免AI腔】：
- 禁止使用"有效降低""显著提升""零失误""高质量""全面"等空泛套话，除非原文确实如此表述
- 语言要简洁务实，像资深职场人写的简历，不像AI生成的模板

【优化标准 - 8项核心规则】：
1. 关键词匹配：在optimized中自然融入JD核心关键词
2. STAR法则：情境→任务→行动→结果
3. 量化数据：保留原文真实数字，不编造，不用占位符
...
- 禁止使用"出色""卓越""高效""有效降低""显著提升""零失误"等主观形容词和空泛套话
- 语言风格要求简洁务实，像资深职场人的简历，不能有明显AI模板痕迹
